...
function renderProgress(){
  ...
  app.innerHTML = `
    ...
    <div class="view">
      ...
      <div class="statbig">
        <div class="s acc"><b>${cur}<span style="font-size:14px"> кг</span></b><span>${t('prog_cur_weight')}</span></div>
      ...
      <div class="chartwrap">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">
          <b class="sm">${t('prog_chart')}</b><span class="xs mut">${ws.length} бичлэг</span>
        </div>
...
function weightChart(ws, base){
  ...
  return `<svg class="chart" viewBox="0 0 ${W} ${H}" preserveAspectRatio="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <text x="${pad}" y="14" fill="var(--mut2)" font-size="11" font-family="Inter">${mx.toFixed(1)}кг</text>
    <text x="${pad}" y="${H-6}" fill="var(--mut2)" font-size="11" font-family="Inter">${mn.toFixed(1)}кг</text>
  </svg>`;
}
